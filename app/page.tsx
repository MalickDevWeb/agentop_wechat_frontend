...
function Brain() {
 ...
 return <div className="content brain"><Header eyebrow="DEVELOPER EXPERIENCE / KNOWLEDGE" title="Explorateur de Cerveau" desc="Interrogez la mémoire validée de votre codebase." action="Partager une règle" onAction={() => { const ev = new CustomEvent("navigate",{detail:"integrations"}); window.dispatchEvent(ev); }} /><div className="brain-search"><Sparkles size={20} /><input placeholder="Posez une question sur l’architecture du projet..." /><kbd>⌘ K</kbd></div>
...
export default function Page() {
  ...
  const View = isDeveloper && screen === 'twin' ? Heal : { twin: Twin, heal: CodeReview, brain: Brain, leaderboard: () => <Leaderboard isAdmin={isAdmin} />, integrations: () => <Integrations userId={user.id} />, components: () => <ComponentHub isDeveloper={isDeveloper} developerTrusted={developerTrusted} onGrantDeveloperTrust={() => setDeveloperTrusted(true)} /> }[screen] || Twin
